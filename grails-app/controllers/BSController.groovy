        ...
        if(session.user) {
            ...
            if(data && data.id) {
                ...
                if(bs) {
                    ...
                    if(bs.validate()) {
                        bs.save()
                        def emails = [bs.agent.courriel, grailsApplication.config.email_sender]
                        if(bs.agent.gu && bs.agent.gu.email) {
                            emails.push(bs.agent.gu.email)
                        }
                        messageService.send(messageService.prepareMessage(session.user, emails, "INRAE - Convocation de " + bs.agent.nom + " " + bs.agent.prenom + " au bilan sanguin le " + bs.sDatePassage, messageService.bsConvocation(bs)))
