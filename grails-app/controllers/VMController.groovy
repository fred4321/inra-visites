                ...
                if(vm.validate()){
                    ...
                    try {
                        ...
                        if(vm.agent.gu && vm.agent.gu.email)
                            messageService.send(messageService.prepareMessage(session.user,  [vm.agent.gu.email], ("[Email non envoyé] INRA - Convocation de "+vm.agent.nom+" "+vm.agent.prenom+" à la visite médicale le "+vm.date.format('dd/MM/yyyy')+"."), "<p>Le message ci dessous n'a pas pu être envoyé à un ou plusieurs des destinaires suivant : "+emails+"</p>"+messageService.vmConvocation(vm)))
                ...
                if (vm){
                    ...
                    if(vm.validate()){
                        ...
                        if(DU){
                            ...
                            messageService.send(messageService.prepareMessage(session.user, [DU], "INRA - Résultat de la Visite médicale de "+ vm.agent.nom + " " + vm.agent.prenom +", le "+ vm.sDate + vm.id, g.render(template:'../pdf/vm', model: [DU:true,vm:vmEmail, user:[profil:"DU"]])))
                        }
                        if(RRH){
                            println "envoi email RRH : "+RRH
                            messageService.send(messageService.prepareMessage(session.user, RRH, "INRA - Résultat de la Visite médicale de "+ vm.agent.nom + " " + vm.agent.prenom +", le "+ vm.sDate + vm.id, g.render(template:'../pdf/vm', model: [DU:true,vm:vmEmail, user:[profil:"SP"]])))
                        }
                        return [status : 200, data : vm]
                    }else{
                        vm.errors.allErrors.each{ println it }
