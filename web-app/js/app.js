...
angular
  ...
  .config([
    ...
    function ($routeProvider) {
      ...
      $routeProvider.when("/programmerVM", {
        templateUrl: "partials/programmerVM.html",
        controller: "ProgrammerVMCtrl",
      });
      $routeProvider.when("/enCourATraiterVM", {
        templateUrl: "partials/enCourATraiterVM.html",
        controller: "EnCourATraiterVMCtrl",
      });
      ...
      $routeProvider.when("/prioritaireBS", {
        templateUrl: "partials/prioritaireBS.html",
        controller: "PrioritaireBSCtrl",
      });
      $routeProvider.when("/programmerBS", {
        templateUrl: "partials/programmerBS.html",
        controller: "ProgrammerBSCtrl",
      });
  ...
  .run([
    ...
    "ReferentielService",
    "FilterModel",
    function ($rootScope, $location, UserService, AgentService, ReferentielService, FilterModel) {
      ...
      ReferentielService.getAll(
        function (result) {
          $rootScope.natures = _.map(result.data, function (item) {
            return { value: item.valeur, text: item.valeur + " - " + item.info1 };
          });
        },
        null,
        { groupe: "natureVM" }
  ...
  .filter("nature", function ($rootScope) {
    return function (value) {
      var found = _.find($rootScope.natures, { value: value });
      if (!found) {
        return value;
      }
