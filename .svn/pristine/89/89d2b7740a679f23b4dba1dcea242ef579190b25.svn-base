...
angular.module('VM', ['ui.bootstrap'])
        .controller('VMCtrl', ['$scope', '$modalInstance', 'items', 'VmService', 'ReferentielService', 'BsService', 'UserService', 'RemarquesService',
            function ($scope, $modalInstance, items, VmService, ReferentielService, BsService, UserService, RemarquesService) {
                $scope.natures = [];
                $scope.typeBS = [];
                $scope.medecins = [];
                $scope.refMedecin = "";
                ...
                $scope.actionBs = function (bs) {
                    if (bs === "true") {
                        ...
                        if (types.length > 1) {
                            ...
                            BsService.add(post, function (result) {
                        ...
                        if ($scope.vm.bs) {
                            BsService.remove($scope.vm.bs, function (result) {
                                $scope.remarques = false;
                                for (var key in $scope.typeBsSelected) {
                ...
                VmService.get(items, function (result) {
                    ...
                    if ($scope.natures.indexOf(result.data.natureVM) !== -1) {
                        $scope.natures.push(result.data.natureVM);
                    }
                    ...
                    $scope.btnGenerer = ['affecté', 'convoqué', 'effectué', 'cloturé'].indexOf(result.data.statut) !== -1 && UserService.getProfil() === 'medecin';
                    //effectué - cloturé
                ...
                ReferentielService.getAll(function (result) {
                    for (var i in result.data) {
                        $scope.natures.push(result.data[i].valeur);
                    }
                ...
                $scope.emailAgent = function () {
                    saveVM(function () {
                        VmService.courriel("mailAgent", {id: $scope.vm.id}, function (result) {
                ...
                $scope.emailRuRrh = function () {
                    saveVM(function () {
                        VmService.courriel("emailRuRrh", {id: $scope.vm.id}, function (result) {
                            if (result.status === 200) {
        ...
        factory('InstanceVM', ['$modal', function ($modal) {
                return function () {
                    this.params = {
